...
fn current_kv_dropped() {
    loom::model(|| {
        let dropped1 = std::sync::Arc::new(0);
        let dropped2 = std::sync::Arc::new(0);

        let map = HashMap::<std::sync::Arc<usize>, std::sync::Arc<usize>>::new();

        map.insert(dropped1.clone(), dropped2.clone(), &epoch::pin());
        assert_eq!(std::sync::Arc::strong_count(&dropped1), 2);
        assert_eq!(std::sync::Arc::strong_count(&dropped2), 2);

        drop(map);

        // dropping the map should immediately drop (not deferred) all keys and values
        assert_eq!(std::sync::Arc::strong_count(&dropped1), 1);
        assert_eq!(std::sync::Arc::strong_count(&dropped2), 1);
    });
}
